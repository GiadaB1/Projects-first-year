...
void inserisci(int sudoku[][NC], char maschera[][NC], int *nvuoti)
{
	...
	scanf("%d",&num);
	if(num >= MIN && num <= MAX && maschera[i][j] != MARKER && *nvuoti != 0)
	{
		sudoku[i - 1][j - 1] = num;
...
{
	int i, ripetizioni, j, isnum, ris, valid,num, is_inf,a,b;
	srand(time(0));
	*nvuoti = 81;
	for(i = 0; i < MAX; i++){
		for(j = 0; j < MAX; j++){
...
{
	int istart, jstart, valid, h, k, conta[MAX];
	istart = i - i % 3;
	jstart = j - j % 3;
	if(*nvuoti){
		valid = -1;
		for(h = 0; h < NR && valid; h++)
			if(sudoku[h][j] != J1 && sudoku[h][j] != J2 && (sudoku[h][j] == sudoku[i][j] && i != h))
				valid = 0;
		for(h = 0; h < NC && valid; h++)
			if(sudoku[i][h] != J1 && sudoku[i][h] != J2 && (sudoku[i][h] == sudoku[i][j] && j != h))
				valid = 0;
		
		for(h = istart; h < istart + 3 && valid; h++)
			for(k = jstart; k < jstart + 3 && valid; k++)
				if(sudoku[h][k] != J1 && sudoku[h][k] != J2 && sudoku[h][k] == sudoku[i][j] && i != h && j != k)
					valid = 0;
	}
	else{
		valid = 1;
